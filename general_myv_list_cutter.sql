...
--and pp.county_name in ('Chelan','Kittitas','Yakima','Douglas','Grant','Benton','Pend Oreille')
...
where phone_rank = 1
and (a.myv_van_id is null or (a.attempts <2 -- should be 1 for first pass or 2 for second and so on
 and (b.ld in ('005','010','024','026','029','030','038','042','044','047') 
or cd in ('003','008'))))
)
/*
select myv_van_id
...
,county_name
from check
--where ld in ('005','010','024','026','029','030','038','042','044','047') 
--or cd = '008'
where (cd in ('001','008') and sbt <= 2430*1)
