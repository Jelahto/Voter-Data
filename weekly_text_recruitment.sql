...
,coalesce(cpmc.phone_number,pp.primary_cell_number) as preferred_phone
...
where (pp.primary_cell_number is not null or (cpmc.phone_number is not null and cpmc.cell_status_id in ('1','2')))

and u.target_subgroup_name in ('Volunteers Tier 3','Volunteers Tier 4')--,'Prospects Tier A','New Leads','Prospects Tier B','Prospects Tier C','Prospects Tier D','Prospects Tier E','Prospects Tier F' )
)

, events_cross_targets as  --not actually a cross join anymore :D
(select t.myc_van_id
,t.first_name
,middle_name 
,t.last_name
