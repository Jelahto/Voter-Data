...
,row_number()over(
  ...
  order by combo_support desc, FARM_FINGERPRINT(b.myv_van_id) desc) as address_rank


from commons.dvc_targets_today b
  join commons.dvc_v5_segments a
...
and b.target_subgroup_name in ('Mobilization Dems','Prio Base Dems') -- or (us_cong_district_latest in ('008') and b.target_subgroup_name = 'Base Dems'))
--and pp.age_combined > 35 --narrowing to older voters for calls or younger for texts
--and pp.voting_address_urbanicity in ('R1','R2','S3')
--and pp.us_cong_district_latest not in ('007','009')
)
,attempted as(
select myv_van_id
...
where phone_rank = 1
and address_rank = 1
and (a.myv_van_id is null or a.attempts < 2) 
...
where (attempts is null 
 or (cd in ('001','008') and sbt <= 2430*1)
or (cd = '002' and sbt <= 2503*.5)
or (cd = '003' and sbt <= 2284*1)
or (cd = '004' and sbt <= 680*.5)
or (cd = '005' and sbt <= 972*.5)
