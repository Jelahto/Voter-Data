    ...
    on db.van_precinct_id = pp.van_precinct_id
      ...
      and cast(tc.checkout_expiration as date) >= current_date("America/Los_Angeles")  
        ...
        and cast(tc.checkout_expiration as date) >= current_date("America/Los_Angeles")
          ...
          and pmc.person_committee_id = '59691'
...
and b.target_subgroup_name in ('Core Dems','Base Dems')
...
,street_address
,city
,st_distance(st_geogpoint(long,lat),st_geogpoint(longitude,latitude))/1609 as distance_miles
,row_number() over(
  partition by location_name
  order by st_distance(st_geogpoint(long,lat),st_geogpoint(longitude,latitude))/1609 asc 
) as dist_rank_event
,row_number() over(
...
from check
  cross join commons.general_gotv_tour_dialer_events 

where include_today is true 
)
/*
select myv_van_id
,first_name 
...
,'myv_van_id'
,phone
,'Nov 2 Core and Base Ellen and Wenatch GOTV Tour Invite'
,'2022-11-02'
,'2022-10-04'
,'calls'
