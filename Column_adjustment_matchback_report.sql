--how can I add the following variables into one column as voted in a previous election in sql?

use the follwoing code as a template:
--create OR REPLACE table reporting.matchback_today_g24 as
with current_status as (
    select distinct *
    from (
        select *
        from democrats.av_ev_wa_20240806_primary.WA_statewide_70_source --redirect to most recent file after upload is processed
    )
)
    ...
    from 
        ...
        left join democrats.analytics_wa.person_votes k
        on pp.person_id = k.person_id
    where 
