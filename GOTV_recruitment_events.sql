--create OR REPLACE table demswasp.commons.GOTV_events_Recruitment as
with targets as 
(
    select 
        u.myc_van_id, 
        ...
        row_number() over(partition by pp.voting_address_id order by farm_fingerprint(pp.person_id)) as address_rank, 
        ar.fo_name
    from commons.vol_tiers_today u  
    join vansync.person_records_myc pmc using (myc_van_id)
    join democrats.analytics_wa.person pp 
        on pp.myv_van_id = pmc.myv_van_id 
        and pp.reg_voter_flag = true
    ...
    and u.target_subgroup_name in ('New Leads', 'Tier A', 'Tier B', 'Tier C', 'Tier D', 'Tier E', 'Tier F')
),
...
(
    select *,
    row_number() over (partition by myc_van_id order by distance_miles asc nulls last) as distance_rank
...
select
    ...
    max(case when type = 'Canvass' then location_name else null end) as canvass_location,
    max(case when type = 'Phonebank' then location_name else null end) as phonebank_location,
    max(case when type = 'Canvass' then location_latitude else null end) as canvass_latitude,
    max(case when type = 'Phonebank' then location_latitude else null end) as phonebank_latitude,
    max(case when type = 'Canvass' then location_longitude else null end) as canvass_longitude,
    max(case when type = 'Phonebank' then location_longitude else null end) as phonebank_longitude
from gotv_tour_events
